  ...
  (let ((accessed '()))
    (define (search-inner still-to-do)
      (if (null? still-to-do)
          #f
          (let ((current (car still-to-do)))
            (if (memq current accessed)
                (search-inner (cdr still-to-do))
                (begin (set! accessed (cons current accessed))
                       (if *search-debug*
                           (write-line (list 'now-at current)))
                       (if (goal? current)
                           #t
...
(define (add-to-index! index key value) ; Index,Key,Val -> Index
  ...
  (let ((new-entry (list key (list value))))
      ...
      ;; entry exists -- insert value if not already there...
        (let ((entry (assv key (cdr index))))
          (if (not (memq value (cadr entry)))
              (set-car! (cdr entry) (cons value (cadr entry)))))))
  index)
...
(define (add-document-to-index! index web url)
  (let ((contents (find-URL-text web url)))
    (define (add-words contents)
      (if (not (null? contents))
          (begin (add-to-index! index (car contents) url)
                 (add-words (cdr contents)))))
    (add-words contents))
)
...
(define (search-web initial-state goal? successors merge graph action)
  (let ((accessed '()))
    (define (search-inner still-to-do)
      (if (null? still-to-do)
          #f
...
(define (BFS-web url goal? web action)
  (search-web url
              goal?
              find-node-children
              (lambda (new old) (append old new))
...
(define (make-web-index web url)
  (let ((web-index (make-index)))
    (begin (BFS-web url
                    (lambda (node) false)
                    web
                    (lambda (url) (add-document-to-index! web-index web url)))
           (lambda (value) (find-in-index web-index value)))))
...
(define (search-any web start-node word)
  (let ((document '()))
    (begin (BFS-web start-node
                    (lambda (node) (memq word (find-URL-text web node)))
                    web
                    (lambda (node) (set! document (find-URL-text web node))))
           document)))
    ...
    (begin (BFS-web start-node
                    ...
                    (lambda (node)
                      (if (memq word (find-URL-text web node))
                          (set! document
                                (cons (find-URL-text web node) document)))))
           document)))
...
;; time where n is the number of entries in optind
  (let ((vector (cdr optind)))
    (define (binary-search begin end k)
      (let ((mid (quotient (+ begin end) 2)))
        (let ((middle (car (vector-ref vector mid))))
          (cond ((= (vector-length vector) 0) false)
                ((eq? k middle) (vector-ref vector mid))
                ((symbol<? k middle) (binary-search begin mid k))
                (else (binary-search (+ mid 1) end k))))))
    (binary-search 0 (vector-length vector) k)))
