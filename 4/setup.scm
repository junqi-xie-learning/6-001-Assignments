...
(define (instantiate-spells)
  (let ((chamber (create-place 'chamber-of-stata)))
    ...
    (create-spell
     ...
     (lambda (caster target)
       (if (ask target 'IS-A 'person)
           (let ((suffering (random 2)))
             (begin (ask target 'EMIT
                         (list (ask target 'NAME) "gets" suffering "points of suffering"))
                    (ask target 'SUFFER suffering caster)))
           (begin (ask caster 'EMIT
                       (list (ask target 'NAME) "is destroyed"))
                  (ask target 'DESTROY)))))
    chamber))

(define (instantiate-counterspells)
  (let ((chamber (create-place 'chamber-of-stata-counter)))
    (create-counterspell
     'boil-spell-counter
     chamber
     'boil-spell)
...
(define chamber-of-stata-counter 'will-be-set-by-setup)
...
(define (setup name)
  ...
  (let ((rooms (create-world)))
    ...
    (set! chamber-of-stata-counter (instantiate-counterspells))

    (populate-spells rooms)
    (populate-counterspells rooms)
    (populate-rings-of-obfuscation rooms)
