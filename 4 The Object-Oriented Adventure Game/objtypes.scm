...
(define (create-ring-of-obfuscation name location) 
  ; symbol, location -> ring-of-obfuscation
  ...
  (let ((mobile-thing-part (mobile-thing self name location)))
    (make-handler
     ...
     (make-methods
      'CASTER
      (lambda ()
        (if (ask (ask self 'LOCATION) 'IS-A 'person)
            (ask self 'LOCATION)
            #f))
      
      'ZAP
      (lambda (target)
        (let ((caster (ask self 'CASTER)))
          (if caster
              (let ((picked-spell (pick-random (ask caster 'HAS-A 'spell))))
                (if picked-spell
                    (begin (ask caster 'EMIT
                                (list (ask caster 'NAME)
                                      "is waving the wand and says"
                                      (ask picked-spell 'INCANT)))
                           (ask picked-spell 'USE caster target))
                    (ask caster 'EMIT
                         (list (ask caster 'NAME)
                               "is waving the wand, but nothing happens")))))))

      'WAVE
      (lambda ()
        (let ((caster (ask self 'CASTER)))
          (if caster
              (let ((target (pick-random (append (ask caster 'PEOPLE-AROUND)
                                                 (ask caster 'STUFF-AROUND)))))
                (ask self 'ZAP target))))))
     mobile-thing-part)))
...
(define (person self name birthplace)
  ...
  'I-feel-better-now)
      ...
      (lambda ()
        (delq self (filter (lambda (person)
                             (not (ask person 'HAS-A 'ring-of-obfuscation)))
     ...
     thing))))
      ...
      (lambda (type)
        (let ((things (ask self 'THINGS)))
          (filter (lambda (thing)
                    (ask thing 'IS-A type)) things)))

      'HAS-A-THING-NAMED
      (lambda (name)
        (let ((things (ask self 'THINGS)))
          (filter (lambda (thing)
                    (eq? (ask thing 'NAME) name)) things)))
...
;; wit-student
;;
(define (create-wit-student name birthplace speed irritability)
  (create-instance wit-student name birthplace speed irritability))

(define (wit-student self name birthplace speed irritability)
  (let ((auto-part (autonomous-person self name birthplace speed 10)))
    (make-handler
     'wit-student
     (make-methods
      ...
      (lambda ()
  (ask auto-part 'INSTALL)
        (create-wand 'wand self)
  (ask clock 'ADD-CALLBACK
       (create-clock-callback 'zap-others self
            'ZAP-OTHERS)))
      'ZAP-OTHERS
      (lambda ()
  (if (ask self 'HAS-A 'wand)
            (let ((wand (car (ask self 'HAS-A 'wand))))
              (if (ask self 'PEOPLE-AROUND)
                  (let ((target (pick-random (ask self 'PEOPLE-AROUND))))
                    (ask wand 'ZAP target))
                  (let ((target (pick-random (ask self 'STUFF-AROUND))))
                    (if target
                        (ask wand 'ZAP target)))))))
      'DIE
      (lambda (perp)
  (ask clock 'REMOVE-CALLBACK self 'zap-others)
  (ask auto-part 'DIE perp)))
     auto-part)))
...
;; wit-professor
;;
(define (create-wit-professor name birthplace speed irritability)
  (create-instance wit-professor name birthplace speed irritability))

(define (wit-professor self name birthplace speed irritability)
  (let ((auto-part (autonomous-person self name birthplace speed 10)))
    (make-handler
     'wit-professor
     (make-methods
  ...
  (if (ask self 'HAS-A 'wand)
      ...
      'TEACH
      (lambda ()
  (let ((students (filter (lambda (person)
                                  (ask person 'IS-A 'wit-student))
                                (ask self 'PEOPLE-AROUND))))
          (if students
              (let ((spell (pick-random (ask chamber-of-stata 'THINGS)))
                    (student (pick-random students)))
                (ask self 'SAY (list "I taught" (ask student 'NAME) (ask spell 'NAME)))
...
;; chosen-one
;;
(define (create-chosen-one name birthplace speed irritability)
  (create-instance chosen-one name birthplace speed irritability))

(define (chosen-one self name birthplace speed irritability)
  (let ((wit-student-part (wit-student self name birthplace speed irritability)))
    (make-handler
     'chosen-one
     (make-methods
      'SUFFER
      (lambda (hits perp)
        (if (> health 0)
            (begin (ask self 'SAY (list "Ouch!" hits "hits is more than I want!"))
                   (set! health (- health hits))))
        (if (<= health 0)
            (begin (ask self 'EMIT (list (ask self 'NAME)
                                         "scar flares brightly and"
                                         (ask perp 'NAME)
                                         "dies instead"))
                   (ask perp 'DIE self)))
  health))
     ...
     (make-methods
      ...
      (lambda (caster target)
        (if (and (ask target 'IS-A 'person)
                 (filter (lambda (counterspell)
                           (eq? (ask counterspell 'COUNTER-SPELL)
                                (ask self 'NAME)))
                         (ask target 'HAS-A 'counterspell)))
            (ask caster 'EMIT (list (ask self 'NAME)
                                    "failed due to a counterspell"))
...
;; counterspell
;;
(define (create-counterspell name location counter-spell)
  (create-instance counterspell name location counter-spell))

(define (counterspell self name location counter-spell)
  (let ((mobile-part (mobile-thing self name location)))
    (make-handler
     'counterspell
     (make-methods
      'COUNTER-SPELL
      (lambda () counter-spell))
     mobile-part)))

(define (clone-counterspell counterspell newloc)
  (create-counterspell (ask counterspell 'NAME)
                       newloc
                       (ask counterspell 'COUNTER-SPELL)))
...
(define (avatar self name birthplace)
  ...
  (let* ((place (ask self 'LOCATION))
    ...
    'OK))
      ...
      'FEEL-THE-FORCE
      (lambda ()
        (let ((people (filter (lambda (person)
                                (not (ask person 'HAS-A 'ring-of-obfuscation)))
                              (all-people))))
          (for-each (lambda (people)
                      (ask screen 'TELL-WORLD (list (ask people 'NAME)
                                                    "is at"
                                                    (ask (ask people 'LOCATION) 'NAME))))
                    people)))
